...
from api_service.logging_config import get_logger, LogCapture

router = APIRouter(prefix="/image", tags=["Image"])

_OUTPUTS_DIR = Path(gateway_config().get("outputs_dir", "outputs")) / "images"
_log = get_logger("image_router")
...
async def _run_image_generation(task_id: str, req: ImageRequest) -> None:
    ...
    _log.log_task_start(task_id, "image_generation", params=req.model_dump())
    ...
    try:
        ...
        _log.log_task_complete(task_id, "image_generation", elapsed * 1000, result=result.model_dump())
    except Exception as e:
        elapsed = time.monotonic() - start
        await mgr.set_failed(task_id, str(e), elapsed)
        _log.log_task_complete(task_id, "image_generation", elapsed * 1000, error=str(e))
...
async def generate_image(req: ImageRequest, background_tasks: BackgroundTasks) -> ImageResponse:
    """
    提交文生图任务。
    默认使用 Z-Image-Turbo 模型 (640x480)。
    - 立即返回 task_id。
    - 通过 GET /image/tasks/{task_id} 轮询结果。
    """
    mgr = get_task_manager()
    task_id = await mgr.create_task("img")
    _log.log_api_request("POST", "/image/generate", task_id=task_id)
    _log.info(f"Image generation request: prompt={req.prompt[:100]}...", task_id=task_id)
    ...
    return ImageResponse(
        ...
        poll_url=f"/image/tasks/{task_id}",
    )
